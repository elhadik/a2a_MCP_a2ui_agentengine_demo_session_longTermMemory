...
| :--- | :--- | :--- | :--- |
| **B1 · Chat Host & MCP Apps** | <ul><li>**Applet rendering**: Pricing tables, Audience Sizing, and Loyalty personalization widgets.</li><li>**HITL checkpoint**: Suspension and resumption via `postMessage`.</li><li>**History & recall**: Load previous events from REST.</li><li>**Conversation management**: Create and delete sessions.</li></ul> | <ul><li>**Streaming**: Chat responses return as single JSON blocks.</li><li>**Attachments**: Cannot upload files (e.g., invoice uploads).</li></ul> | <ul><li>**Streaming**: Refactor the FastAPI chat endpoint to return a `StreamingResponse` using Server-Sent Events (SSE) to render text tokens in real time.</li><li>**Attachments**: Add an upload button in the chat interface that uploads files to a GCS bucket, passing the file metadata/URI to the Supervisor.</li></ul> |
| **B2 · Prompt Orchestration** | <ul><li>**Multi-part decomposition**: Coordinated hub-and-spoke flow.</li><li>**Structured step chaining**: Sequential execution.</li><li>**Mid-flow checkpoints**: Handled by A2UI callbacks.</li></ul> | <ul><li>**Plan transparency**: The user does not see what steps the orchestrator is planning next.</li><li>**Long-running queries**: UI locks during long MCP database queries.</li></ul> | <ul><li>**Plan transparency**: Let the Supervisor output its planned execution steps as metadata, and display them as a progressive checklist in the UI (e.g., *"Sizing audience segment... [In Progress]"*).</li><li>**Long-running queries**: Implement asynchronous polling for MCP calls, where the tool returns a `job_id` and the UI polls for completion.</li></ul> |
